...
同一工作区使用一个当前岗位，适合招聘者切换处理多个岗位。操作前可直接说“切换到 AI 产品经理岗位”。确认写回脚本会核对岗位和提案，并通过工作区锁避免并发写入；多个对话需要同时独立操作时，应使用独立工作区。
...
`.recruitment-agent/` 保存当前岗位、待确认提案及写入备份；`AGENT_PROTOCOL.md` 定义统一运行规则。真实岗位、候选人、提案、备份及默认导出目录被 `.gitignore` 排除。聊天记录用于临时交流，长期事实以岗位文件、台账和操作日志为准。
