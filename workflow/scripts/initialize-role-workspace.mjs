...
export async function initializeRoleWorkspace({ rootPath, roleName, pipeline, capacity, documents = {}, templateRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../templates") }) {
...
if (process.argv[1] && pathToFileURL(path.resolve(process.argv[1])).href === pathToFileURL(currentFile).href) {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log("用途：创建岗位工作区。\n用法：node workflow/scripts/initialize-role-workspace.mjs --root <项目根目录> --role <岗位名称> --pipeline <PIPELINE.json> [--documents <已确认文档.json>] [--capacity <人数>]");
    process.exit(0);
  }
  ...
  console.log(JSON.stringify(await initializeRoleWorkspace({ rootPath, roleName, pipeline, capacity, documents }), null, 2));
}
