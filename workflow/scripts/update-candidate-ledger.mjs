...
export async function upsertCandidates({ ledgerPath, candidates }) {
  ...
  for (const candidate of candidates) { const patch = mergeCandidate(candidate); let row = null; sheet.eachRow((current, rowNumber) => { if (rowNumber >= 4 && current.getCell(indexes.get("候选人ID")).value === candidate.candidateId) row = current; }); if (!row) { for (let n = 4; n <= sheet.rowCount; n += 1) { const current = sheet.getRow(n); if (![...indexes.values()].some(column => String(current.getCell(column).value ?? "").trim())) { row = current; break; } } } if (!row) throw new Error("Candidate ledger has no empty rows available. 请重新创建更大容量的台账。"); Object.entries(patch).forEach(([key, value]) => row.getCell(indexes.get(key)).value = value ?? ""); }
...
if (process.argv[1] && pathToFileURL(path.resolve(process.argv[1])).href === pathToFileURL(currentFile).href) {
  const message = "旧版直写命令已停用。请使用 create-pending-action.mjs 生成预览，人工确认后调用 apply-confirmed-action.mjs，以确保档案、日志和看板同步。";
  if (process.argv.includes("--help") || process.argv.includes("-h")) console.log(message);
