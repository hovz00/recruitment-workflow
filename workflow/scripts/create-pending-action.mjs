...
if (process.argv.includes("--help") || process.argv.includes("-h")) {
  console.log("用途：保存待确认的候选人变更提案。\n用法：node workflow/scripts/create-pending-action.mjs --root <项目根目录> --input <proposal.json> [--session <会话编号>]");
  process.exit(0);
}
const rootPath = argument("--root");
const input = argument("--input");
if (!rootPath || !input) throw new Error("用法：--root <项目根目录> --input <proposal.json>");
console.log(JSON.stringify(await createPendingAction({ rootPath, sessionId: sessionArgument(), proposal: JSON.parse(await fs.readFile(input, "utf8")) }), null, 2));
