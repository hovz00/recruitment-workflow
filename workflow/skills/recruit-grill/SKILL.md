...
面试阶段明确配置 `kind: "interview"` 与预约字段，尤其是案例答辩等自定义名称。完整文档与流程获得招聘者明确确认后，初始化时传入确认版本、确认人和依据，或调用 `confirm-role-standard.mjs` 记录 `ROLE_CONFIRMATION.json`。标准/流程修改后重新确认；草稿不能开始候选人写回。
