...
test('standard edits invalidate pending score decisions before any write',async t=>{const f=await fixture(t);await apply(f,intake());const p=await createPendingAction({rootPath:f.rootPath,proposal:{...intake(),intent:'candidate_update',changes:[{field:'能力证据得分',before:'',after:8}]}});const before=await fs.readFile(path.join(f.rolePath,'candidate-ledger.xlsx'));await fs.appendFile(path.join(f.rolePath,'ROLE_STANDARD.md'),'\n业务目标已调整。');await assert.rejects(()=>applyConfirmedAction({rootPath:f.rootPath,proposalId:p.id}),/标准.*变化|标准.*确认/);assert.deepEqual(await fs.readFile(path.join(f.rolePath,'candidate-ledger.xlsx')),before);});
...
test('invalid merged fields cannot change ledger, archive or dashboard',async t=>{
  ...
  await apply(f,{...intake(),intent:'candidate_update',changes:[{field:'能力证据得分',before:'',after:0},{field:'证据覆盖率',before:'',after:100},{field:'1-案例答辩预约日期',before:'',after:'2999-01-01'}]});
